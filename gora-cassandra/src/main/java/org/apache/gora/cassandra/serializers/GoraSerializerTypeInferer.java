...
public class GoraSerializerTypeInferer {
  ...
  public static <T> Serializer<T> getSerializer(Schema schema) {
    ...
    switch (type) {
      case STRING:  return (Serializer<T>) Utf8Serializer.get();
      case BOOLEAN: return (Serializer<T>) BooleanSerializer.get();
      case BYTES:   return (Serializer<T>) ByteBufferSerializer.get();
      case DOUBLE:  return (Serializer<T>) DoubleSerializer.get();
      case FLOAT:   return (Serializer<T>) FloatSerializer.get();
      case INT:     return (Serializer<T>) IntegerSerializer.get();
      case LONG:    return (Serializer<T>) LongSerializer.get();
      case FIXED:
        Class clazz = TypeUtils.getClass(schema);
        return (Serializer<T>) SpecificFixedSerializer.get(clazz);

      case ARRAY: return GenericArraySerializer.get(schema.getElementType());
      case MAP:   return StatefulHashMapSerializer.get(schema.getValueType());
      case UNION: 
      case RECORD: return (Serializer<T>) AvroSerializer.get(schema) ;
      case NULL: return (Serializer<T>) NullSerializer.get();
      default:
        return null ;
    }
  }
  ...
  /**
   * Get serializer for when we only know the type.
   * @param type Type, only allowed {STRING, BOOLEAN, BYTES, DOUBLE, FLOAT, INT, LONG, FIXED}
   * @return
   */
  ...
  public static <T> Serializer<T> getSerializer(Type type) {
    switch (type) {
      ...
      case INT:     return (Serializer<T>) IntegerSerializer.get();
      case LONG:    return (Serializer<T>) LongSerializer.get();
      case FIXED:   return (Serializer<T>) SpecificFixedSerializer.get();
      default:
        return null ;
    }
  }
