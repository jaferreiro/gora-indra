...
public class PersistentDatumReader<T extends PersistentBase>
  ...
  @SuppressWarnings("unchecked")
  public Object cloneObject(Schema schema, Object fromInstance, Object reuseInstance) {
    ...
    switch(schema.getType()) {
      case MAP    :
        Map<Utf8, Object> map = (Map<Utf8, Object>)newMap(reuseInstance, 0);
        
        if (schema.getType() == Type.RECORD) {
          for(Map.Entry<Utf8, Object> entry: ((Map<Utf8, Object>)fromInstance).entrySet()) {
            map.put((Utf8)createString(entry.getKey().toString()), ((Persistent)entry.getValue()).clone());
          }
        } else {
          for(Map.Entry<Utf8, Object> entry: ((Map<Utf8, Object>)fromInstance).entrySet()) {
            map.put((Utf8)createString(entry.getKey().toString()), cloneObject(schema.getValueType(), entry.getValue(), null));
          }
        }
        return map;
      case ARRAY  :
        GenericArray<Object> array = (GenericArray<Object>) newArray(reuseInstance, (int)((GenericArray<?>)fromInstance).size(), schema);
        
        if (schema.getType() == Type.RECORD) {
          for(Object element: (GenericArray<Object>)fromInstance) {
            array.add(((Persistent)element).clone());
          }
        }else{
          for(Object element: (GenericArray<Object>)fromInstance) {
        ...
        return array;
      case RECORD : return ((Persistent)fromInstance).clone();
      case STRING : return createString(fromInstance.toString());
      default     : return fromInstance; //shallow copy is enough
    }
  }
}
